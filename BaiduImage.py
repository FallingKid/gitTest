    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 对名字重复的图片，后面加上（1）、（2）...（...）
                ...
                Name_repeat[image['title']] = 1 if not Name_repeat.get(image['title']) else Name_repeat[image['title']]+1
                path_image = "{0}/{1}（{2}）.jpg".format(Path, image['title'], Name_repeat[image['title']])
                with open(path_image, 'wb') as f:
                    f.write(response.content)
    except requests.ConnectionError:
